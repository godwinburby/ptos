...
### Routines page improvements (day-view timeline + delete)

- **Overdue routines no longer look done** — the false strike-through (`text-decoration:line-through`) was removed from `.routine-item.past` (card view) and `.rv-block.past` (day view). Past routines keep only the faded `opacity`, so an overdue-but-open routine reads as "past", never "completed". Completed routines still strike through via the `.completed` styles.
- **Timeline trimmed to the routine span** — the day view no longer renders the full 00:00–23:00 grid. `first_hour`/`last_hour` are now computed from the earliest and latest routines (latest block end rounded up to the next hour, e.g. a 06:45 + 20:15 span renders 6:00→21:00). Block positioning, `total_height`, and `now_top` all follow the new bounds.
- **Now-line guard** — the red "Now" indicator renders only when the current time falls inside the trimmed timeline (`show_now`); it is hidden when it would land outside the visible range (e.g. mid-afternoon with only a morning routine).
- **Delete buttons on the routines page** — a ✕ button on each card-view row, day-view block, and Anytime row calls `deleteTodo(line_no)` (confirm → `POST /todo/delete` → reload). The shared edit modal also gains a red **Delete** button when included via `show_delete=True` (routines page only); it routes through `#modal-target` to `/todo/delete` or `/todo/delete-done`, and only shows in edit mode. The Todos page modal is unchanged.
- **Clock extraction for tests** — `routines_page` now reads the time via module-level `_now_time_str()`, so day-view tests can pin the clock without patching `datetime.datetime.now` (immutable on Python 3.13).
- **`R` keyboard shortcut** — the `R` key navigates to the Routines page (added to the single-key navigation set in `base.html` and the help overlay's Quick Navigation table). The sidebar nav hint for Routines is now `R` (it previously advertised a non-functional `G Z`).
- **Types shortcut changed to `Y`** — Record Types moved from the `G X` chord to the single-key `Y` nav set: the sidebar hint is now `Y`, a bare `Y` keypress navigates to `/types`, and the help overlay's Quick Navigation table lists `Y → Record Types` (removed from Extended Navigation). The `G Y` chord for Calendar is unaffected.
- **Project-based day-view colors** — day-view timeline blocks are now color-graded by each routine's first **non-`+routine` project** (e.g. `+gym`) instead of its `@context`, so routines sharing a project share a block color. A routine with no extra project renders as neutral gray (`ctx-other`); the palette is assigned in first-seen order across the day's routines. A compact **legend** below the timeline maps each project (and `other`) to its color. Card grouping is unchanged — cards still group by `@context`. Help text updated to match.
- **Project-based card-view colors** — card-view and Anytime rows now also carry their routine's project color (`row_colors`, keyed by `line_no`, from the same first-seen palette as the day view). A 3px colored left accent bar + the matching `ctx-*` tint identifies the row's project; rows keep a neutral row separator. Done rows stay neutral (fade-only). The legend moved out of the day-view block to a spot above both views, so it renders in cards and day views alike (shown only when at least one routine exists). Help text broadened to "colors follow the project in both cards and the day view".
...
- **`--migrate-log-group TYPE` CLI command** (`ptos_cli.py`): explicit one-time migration command that finds every `type=TYPE` line across `records/*.log`, removes it from the source file, and appends to `records/<group>/<year>.log`. Uses `append_record()` (the same proven write path as `--add`) for reliable atomic writes to subdirectories. Requires `log_group` to be set in schema. Does not run automatically.
...
- **"Create new type…" on convert page** (`edit.html`): the target type `<select>` on the convert page now has a "+ Create new type…" option at the bottom. Selecting it redirects to `/types/new` with the source note and a `return_to` back to the convert page. After type creation, the redirect injects `target_type=<name>` so the new type is pre-selected on return.
- **`types_post` return_to injection** (`ptos_web.py`): when `return_to` is a convert page URL and a new type was created, `target_type=<name>` is appended so the convert page pre-selects the new type.
