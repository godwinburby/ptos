...
- `POST /board/move` — status-mode drag: `{board, filepath, line, lineno, target_lane}` → `svc.board_move_record()` rewrites the record's `set_field` in place to the target lane's `set_value`. Server resolves `set_field`/`set_value` from queries.toml (never trusts the client); raises `PTOSError` for a non-status board, unknown lane, unresolvable set value, or a filepath escaping `RECORDS_DIR`. Returns `{ok, new_line}`; the page reloads
