...
- **Tests**: `tests/test_convert.py` (59 tests — draft rules, write/keep/delete + cache invalidation, guesser scoring/determinism, scraper amounts/options/tags/history-defaults + strip_spans, note scrub + connector rule + option stripping + idempotency + currency rs/₹, CLI suggestion auto-pick + explicit target + `--keep` + `--keep-note` + blocked-write guard + confirm deny + filter requirements + backlink warning, web GET/POST convert incl. missing-required re-render + prefill-amount-from-source + empty-note-clears).
...
- **Home page Capture card** — `web_templates/home.html`: card + input + `doCapture()` JS (POST `/api/capture` `{text}`, clears input, flashes `✔ Captured` / error); placed above the `{% if frequent_presets %}` Quick Add card, always visible
