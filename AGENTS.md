...
- **New type suggestion from free text** — `suggest_new_type(note, schema=None)` analyses free text for action words (`_ACTION_WORDS` dict: bought→purchase, walked→activity, called→call, cooked→meal, met→meeting, wrote→document, booked→booking, repaired→repair, etc.; no match → `note`), currency amounts (→ `amount` int required field), `@tags` (→ `category` string field with options), duration patterns (`N minutes`/`hours` etc., → `duration` int field), and person references (capitalised word after `for`/`with`/`to`/`from`/`by`/`of`/`about` → `person` string field). Returns `{name, fields}` or `None` when the text has no signals or the suggested type name already exists in the schema. Purely advisory — the caller confirms. `create_type_from_suggestion(spec)` creates the type via `ptos.add_type()` + `add_type_field()` for non-required fields; `create_and_convert(note, filepath, old_line, lineno, target_name, spec, keep, strip_note)` chains creation + `convert_record(keep=True)`. Capture marking with `converted=<target_type>` is handled by `convert_record` itself (when `keep=True` and source is a capture).
- **CLI** — `--convert "WHERE..."` omit TARGET: when `suggest_convert_type()` returns empty, falls back to `suggest_new_type()`. If a suggestion is found, prints the proposed type and fields, prompts `Create type and convert? [y/N]` (auto-confirms in non-TTY). On confirm, creates the type then converts all matched records. Prints `Source record kept (marked as converted to <type>).` after each conversion.
- **Web** — `_convert_render_kwargs()` calls `suggest_new_type()` when convert suggestions are empty or all below 50%, passes `convert_new_type_sug` and `convert_already_converted` (from source kv's `converted` field) to the template. A "Create & Convert" card appears below the CONVERTING card when a suggestion exists; the button POSTs `create_and_convert=1` which creates the type then converts. `POST /api/suggest-type` route returns a suggestion for AJAX use. A yellow warning banner appears on the convert page when the source record already has `converted=<type>`.
...
- **Tests**: `tests/test_convert.py` (116 tests — draft rules, write/keep/delete + cache invalidation, guesser scoring/determinism, scraper amounts/options/tags/history-defaults + date expression recognition + strip_spans, note scrub + connector rule + orphan boundary + option stripping + idempotency + currency rs/₹, CLI suggestion auto-pick + explicit target + `--keep` + `--keep-note` + blocked-write guard + confirm deny + filter requirements + backlink warning, web GET/POST convert incl. missing-required re-render + prefill-amount-from-source + empty-note-clears, new-type suggestion action-word inference + amount/tag/duration/person extraction + edge cases + create_type_from_suggestion, _mark_converted before-pipe/without-pipe/idempotent + convert_record marks kept captures + non-capture no-mark + deleted no-mark).
