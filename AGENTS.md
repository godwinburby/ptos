...
- **Daily digest** — `svc.daily_digest(date=None)` is a **pure read** (no writes) composing existing engine functions: `scan_records` for the date (grouped `records_by_type` with counts + up to 5 compact `_digest_sample` lines), `ptos_todo.load_todos` for **overdue** and **due** lists (`t.due < date` / `== date`; done and threshold-hidden skipped; capped at 60), `find_records_with_location(["type=capture"], start=date.min, end=date)` for recent captures (recent-first, capped at 10, record dates > digest date excluded), `journal_path` for the date's journal preview (none when missing), and `get_habit_data` per configured habit for `{name, streak, days_done, today, range_label}`. Default date = **yesterday**; accepts `today`/`yesterday`/`YYYY-MM-DD`. Returns `{date, weekday, records_by_type, todos, captures, journal, habits}`. CLI `--daily [DATE]` (const `"yesterday"`); web `/daily` + `/daily/<date>` with prev/next nav
