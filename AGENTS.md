...
- **CLI** — `--board [NAME]` (`run_board(arg)`): prints `Board: NAME  (window: ...)` then per-column `TYPE: N record(s)` (with `/shown` when truncated) and record lines `[date]  <card-title fields>  <note>`, capped at 15 per lane; no name → hint for the `["board.NAME"]` config; unknown name → friendly exit
