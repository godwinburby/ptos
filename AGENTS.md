...
- Optional `match_field` config key (e.g. `match_field = "client_code"` on `["board.client_sale_journey"]`) enables **cross-column color highlighting**: records across the board's columns that share the same `match_field` value get the **same color**, provided that value appears in **≥2 distinct columns** (a lone record with no sibling in another column stays uncolored — so a prescription with no fitting shows uncolored)
- **Fully generic / schema-agnostic**: the engine/service contain zero field/type/schema assumptions — the only field name is whatever a board's `match_field` config supplies; no `match_field` (or a blank one) → feature inactive (`get_board_data` returns `match_field=None`, no colors attached)
- `get_board_data()` computes a `value → color` map over the FULL per-column record set (before `limit` truncation, same as rollups); colors are assigned by **sorted order over the visible matched set** (distinct matched values sorted, `index % 16`) into the 16-color board palette (`accent`/`purple`/`teal`/`rose`/`slate`/`warn`/`success`/`error`/`indigo`/`cyan`/`lime`/`amber`/`pink`/`brown`/`navy`/`olive`); each matching record gets `_link_color` + `_link_group` (the matched value); `match_field` returned in board data. **No color overlap** for up to 16 distinct matched codes in one view (different codes never share a color); with >16 distinct codes color reuse is unavoidable (fundamental limit). Colors are deterministic for the same code set (same view = same mapping)
- `board.html`: `hl-{color}` classes (colored left border + light background tint) applied to cards, plus a colored dot + matched-value badge on the card; Query Builder board editor has a free-form **Match Field** text input; `save_queries_full()` persists `match_field` (like `rollup_field`)
- CLI parity (`run_board`): prints `match_field: <field>` header and a colored token per matched record; no-`match_field` boards print as before

### Client grid view (row-per-matched-value alignment)

- `/board` gains a **view toggle** (`Kanban` / `Client grid`) shown only when the board sets `match_field`. `?view=kanban` (default) renders the classic kanban exactly as before (with drag-and-drop); `?view=grid` renders the **row-per-client grid**, where records sharing a `match_field` value **align horizontally across columns** so the whole client journey is scannable left-to-right (row label = the matched value with its color dot, one cell per board column containing that client's stacked cards — reusing the existing card markup + `hl-{color}` — or a dashed placeholder when the client has no record there)
- `get_board_data()` additionally returns `grid_rows` (ordered `[{value, color, cells:{col_type:[records]}}]` built from the **displayed** records, sorted by value matching the color order), `unmatched` (`{col_type:[records]}` for lone/missing-value records), and `has_matching` (bool). These are **additive** — the existing kanban keys (`data`, `counts`, `rollups`, ...) are unchanged, so kanban rendering/tests are intact
- Matched values are exactly the ones colored (appear in **≥2 distinct columns**); single-column/missing-value records go to the **Unmatched** section rendered as per-column groups below the grid. **Drag-and-drop in grid view**: a **matched** card (inside a `.bg-row`) can only be dropped into another stage cell on **the same client row** (the row's empty placeholders included); dropping on a different client's row is rejected (rows are keyed uniquely by the `match_field` value, so "same row" == "any row with the same matching field"). An **unmatched** card (no row, in the Unmatched section) can be dropped into **any** board column (kanban-style, filling the gap). Drag is implemented by sharing the kanban handlers generalized to work on `.bg-cell`/`.bg-unmatched-lane` via `_sourceType`/`_dropType`/`_dropEl`, with `_dragData.srcRow` enforcing the same-row rule in `onDragOver`/`onDrop`
- `ptos_web.py` `/board` reads/validates `view`; `board.html` `switchView()` + `updateUrl()` preserve `view` alongside `board`/time params; new CSS classes `.bg-grid`/`.bg-table`/`.bg-head`/`.bg-row`/`.bg-label`/`.bg-cell`/`.bg-placeholder`/`.bg-unmatched*` (plus `.bg-cell.drag-over`/`.bg-unmatched-lane.drag-over` drop highlights). `POST /board/advance` now takes only `line`/`lineno`/`target_type`
- **Advance date simplification**: `advance_record()` (and thus every board drag/advance) always keeps the **source record's original `YYYY-MM-DD`** — no window-aware dating. The `_advance_target_date()` helper and `board_name`/`win_time`/`win_from`/`win_to` params were removed, along with `_currentWindowParams()` in `board.html`. This is the single simple rule: the advanced card always stays in the period it was dragged from
...
- `POST /board/advance` — creates a new record of target type, copies shared fields; returns `missing_required` + `new_line`/`new_filepath`/`new_lineno` for edit redirect; the new record always keeps the **source record's original `YYYY-MM-DD`** so an advanced card stays in the period it was dragged from
- `GET /board` — renders Kanban view, board selected via `?board=NAME` query param; auto-selects first board if none specified
- **CLI** — `--board [NAME]` (`run_board(arg)`): prints `Board: NAME  (window: ...)` then per-column `TYPE: N record(s)` (with `/shown` when truncated) and record lines `[date]  <card-title fields>  <note>`, capped at 15 per lane; when the board sets `match_field`, also prints a `match_field: <field>` header and a colored `[<color> field=value]` token per matched record; no name → hint for the `["board.NAME"]` config; unknown name → friendly exit
...
- **Caching** — `get_habit_data` cached per habit + window under `habit:{name}:{time|'tm'}:{from_date}:{to_date}` (the default window shares the `tm` key); `_invalidate_history_cache()` pops `habit:` keys alongside `history:`/`condsug:` so all 7 record-write paths already invalidate habits (no new call sites)
...
- **Tests**: `tests/test_habits.py` (streak consecutive/gap/today-missing, double-log single present, cache invalidation on append, no-rescan on repeat call, unconfigured raises; `TestHabitWindow` covers Monday-aligned grid, `is_today`, month-label columns, `time="tm"`, from/to range, past window no-today, all-time column cap; `TestHabitMonths` covers per-month block shape/alignment/padding; `TestHabitCli` covers `--habits` output, name filtering, missing-name exit, no-habits hint; `TestHabitToggle` covers add/remove/unconfigured/future/invalid-date/streak-update/double-toggle-idempotent/web-route/web-missing-fields/web-unconfigured, toggleable default true, toggleable false in data, non-toggleable rejection (service + web))
...
- **CLI** — `--convert "WHERE..."` omit TARGET: when `suggest_convert_type()` returns empty, falls back to `suggest_new_type()`. If a suggestion is found, prints the proposed type and fields, prompts for name override (`Type name [suggested]:`), per-field keep/drop (`Keep field 'X' (type)? [Y/n]`), then `Create type and convert? [y/N]` (non-interactive skips with a message). On confirm, creates the type then converts all matched records. Prints `Source record kept (marked as converted to <type>).` after each conversion.
- **Web** — `_convert_render_kwargs()` calls `suggest_new_type()` when convert suggestions are empty or all below 50%, passes `convert_new_type_sug` and `convert_already_converted` (from source kv's `converted` field) to the template. A "Create & Convert" card appears below the CONVERTING card when a suggestion exists; the card has editable inputs for type name and fields (name/type/required per field, add/remove buttons). The button POSTs `create_and_convert=1` with the edited spec (`new_type_name` + `new_type_fields` JSON) which creates the type then converts. `POST /api/suggest-type` route returns a suggestion for AJAX use. A yellow warning banner appears on the convert page when the source record already has `converted=<type>`. `doCreateAndConvert` JS parses the server response for the error `.msg` element and shows it in an alert on failure (instead of silently reloading).
- **CLI** — `--convert "WHERE..." TARGET [--set key=value ...] [--keep] [--keep-note] [--all]` (`run_convert`, dispatched before `--set/--delete` since it also takes `--set`): sugar filters work exactly like `--set` (a filter token that IS a valid type name as the last arg is treated as the target; single-token target → "needs a filter" exit; no filters at all → require-filter exit). **Omit TARGET** → suggestion mode: prints top 5 guessed types with `pct`, auto-picks the top one when non-TTY (`sys.stdin.isatty()`), else interactive pick. `--set` overlays: `key=value`, `key=` (clear), `tag+=`/`tag-=` (only `tag` supports `+=`/`-=`; `id`/`links` rejected). `--keep-note` opts out of the note scrub (source note carries verbatim). Multi-match pick-by-number prompt (or `--all`); missing required fields print as `[blocked] target type requires: ...` and abort before any write; delete path warns via `ptos.backlink_refs(f"{rtype}:{rid}")` ("N entrY/ies link to ... will become dangling"); final `Convert? [y/N]` confirm (EOF-safe via `_cli_input`); per-record `Converted:` output.
- **Web** — `edit_get` with `?convert=1` renders the convert screen (`/edit?convert=1&filepath=...&lineno=...&line=...&return_to=...`): target `<select>` of `allowed_types` minus the source type with `· pct%` suggestion markers (`target_type` URL param wins, else top suggestion ≠ source, else first allowed ≠ source), amber "CONVERTING X → Y" card noting id/links carry rules, scrape-prefilled fields, "Prefilled from the note: ..." line, `remove_original` checkbox (checked by default), save button labeled "Convert Record". `edit_post` with form `convert=1` rebuilds `ov` from the form's required/fields/conditions + global fields (`spaces → _`), `tag` from `getlist("tag")` + `custom_tags`, optional `date`/`note` overrides, then `svc.convert_record`; on `PTOSError` it re-renders the convert page with the error. Both GET/POST and the normal edit path render through the shared `_render_edit()` helper (`_convert_render_kwargs()` computes the convert-mode extras). When `create_and_convert=1` is posted, creates the suggested type then converts (passing `kv_overrides=ov`); if the type already exists, creation is skipped and conversion proceeds directly. On "Convert blocked" (missing required fields), redirects to the conversion page instead of showing an error — the type was already created so the page can render its fields for the user to fill in.
- **Record tables** — `record_table.js` gains a `⇄` anchor (warn-colored, "Convert to another type") per row linking to the convert page; include bumped to `?v=3` on browse/home/query_builder/queries (SW `ptos-v3` cache).
- **Tests**: `tests/test_convert.py` (119 tests — draft rules, write/keep/delete + cache invalidation, guesser scoring/determinism, scraper amounts/options/tags/history-defaults + bare-word tag scraping + date expression recognition + strip_spans, note scrub + connector rule + orphan boundary + option stripping + idempotency + currency rs/₹, CLI suggestion auto-pick + explicit target + `--keep` + `--keep-note` + blocked-write guard + confirm deny + filter requirements + backlink warning, web GET/POST convert incl. missing-required re-render + prefill-amount-from-source + empty-note-clears, new-type suggestion action-word inference + amount/tag/duration/person extraction + edge cases + create_type_from_suggestion, _mark_converted before-pipe/without-pipe/idempotent + convert_record marks kept captures + non-capture no-mark + deleted no-mark).

## Capture, Pomodoro logging, and Daily digest (phase-1 trio)
...
- **Daily digest** — `svc.daily_digest(date=None)` is a **pure read** (no writes) composing existing engine functions: `scan_records` for the date (with full parsed records + `_filepath`/`_lineno`/`_line` metadata for RecordTable, plus grouped `records_by_type` summary), `ptos_todo.load_todos` for **overdue** and **due** lists (full todo dicts with `id`, `links`, `rec`, `threshold`, `threshold_time` for the edit modal), `find_records_with_location(["type=capture"], start=date.min, end=date)` for recent captures (with record metadata for edit/delete), `journal_path` for the date's journal preview (none when missing), and `get_habit_data` per configured habit for `{name, streak, days_done, today, range_label}`. Default date = **yesterday**; accepts `today`/`yesterday`/`YYYY-MM-DD`. Returns `{date, weekday, records, columns, records_by_type, todos, captures, journal, habits}`. CLI `--daily [DATE]` (const `"yesterday"`); web `/daily` + `/daily/<date>` with prev/next nav. The web page is fully interactive: records use `RecordTable` (edit/delete/convert/bulk), todos have inline complete/edit/delete with the full edit modal (priority picker, date pickers, projects/contexts chips), captures have edit/delete buttons. Shared todo row CSS (`.todo-row`, `.todo-check`, `.todo-pri`, `.todo-body`, `.todo-desc`, `.todo-meta`, `.todo-due-badge`, `.todo-project`, `.todo-context`, `.todo-actions`) plus modal/field-popup CSS lives in `base.html` for reuse across pages
...
- **Tests**: `tests/test_capture.py`, `tests/test_pomodoro.py`, `tests/test_digest.py` (service append/type-missing/disabled/bad-input, CLI prints/exits, web JSON routes, digest grouping/todo-threshold/capture-boundary/journal/habits, default-yesterday and explicit-date)
