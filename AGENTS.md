...
- Keys: `columns`, `time_window` (default `this-month`), `limit` (default 0 = unlimited), `card_title_fields` (comma-separated string or list of field names for card header), `rollup_field` + `rollup_op` (`sum`/`avg`/`count`, default `count`) for lane rollups, `match_field` (optional field name for cross-column highlighting), `set_field` (optional field name that switches the board to **status mode**), `stamp_field` (optional field name written to today's date in the same atomic write whenever a status-board card is dragged — a "when did this last move" timestamp; boards without it write only the `set_field` change, exactly as before; persisted by `save_board`/`save_queries_full`, exposed on the `get_board_data()` return dict, and read by the Project Drift Review board-stall calculation to age cards from the stamp instead of the record date).
- **Two board modes, one grammar** — `columns` items are either a bare **type string** (type mode, the default) or a **dict** `{label, where, set?}` (status mode, requires `set_field`). `set_field` present ⇒ **status board** (all lanes must be dicts; dragging rewrites the card's `set_field` in place — one record stays one card); absent ⇒ **type board** (all lanes strings; dragging calls `advance_record`, which creates a NEW record of the target type and keeps the source). Modes are mutually exclusive per board: a dict lane without `set_field`, or a string lane with `set_field`, both raise `PTOSError`. Dict lanes: `label` (unique lane key + display name), `where` (a single condition or an explicit `AND`/`OR`/`NOT`/paren expression — space-separated conditions without a keyword do NOT parse, use `AND`), optional `set` (explicit drag value; otherwise derived from the lane's single plain `set_field=value` in `where`). Lanes whose set value can't be resolved (e.g. multi-value `status=hired|rejected` without `set`, `~` contains, comparisons) render normally but are **not drop targets**
- Board editor in Query Builder: drag-reorderable column chips (type mode), or full **status-lane editor** with `set_field` input + per-lane label/where/set inputs with ▲▼/✕ controls (status mode), plus time window/max cards fields, card title priority chip picker, rollup field/op dropdowns
...
- `POST /board/advance` — type-mode drag: creates new record of target type, copies shared fields; keeps source record's original date
- `POST /board/move` — status-mode drag: `{board, filepath, line, lineno, target_lane}` → `svc.board_move_record()` rewrites the record's `set_field` in place to the target lane's `set_value`, writing the board's `stamp_field` (if configured) to today's date in the **same** atomic write. Server resolves `set_field`/`set_value`/`stamp_field` from queries.toml (never trusts the client); raises `PTOSError` for a non-status board, unknown lane, unresolvable set value, or a filepath escaping `RECORDS_DIR`. Returns `{ok, new_line}`; the page reloads
- `GET /board` — renders Kanban view, board selected via `?board=NAME`; auto-selects first board if none specified
- **CLI** — `--board [NAME]`: prints per-column records with lane labels (status boards show `set_field` in the header); when `match_field` set, prints colored tokens per matched record
